...
Kernel::Kernel(int argc, char **argv)
{
  ...
  NumPhysPages = 32;         // number of physical memory pages
  TLBSize = 4;               // if there is a TLB, make it small
  millisPerTick = 0.001;
  ...
  hostName = 1;               // machine id, also UNIX socket name
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-rs") == 0) {
      ...
      ASSERT(i + 1 < argc);   // next argument is int
      NumPhysPages =atoi(argv[i + 1]);	// set number of physical pages
      ASSERT(NumPhysPages >= 2);          // guarantee at least 2 pages
    } else if (strcmp(argv[i], "-tlb") == 0) {
      ASSERT(i + 1 < argc);   // next argument is int
      TLBSize =atoi(argv[i+ 1]);	        // set number of physical pages
      ASSERT(TLBSize >= 2);               // guarantee at least 2 pages
