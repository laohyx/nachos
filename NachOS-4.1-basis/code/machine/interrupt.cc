...
Interrupt::Interrupt()
{
    getRealTicks();  // intitializes calculation of milliseconds since start
    level = IntOff;
...
//----------------------------------------------------------------------
// SynchronizeWithRealTime
// 	Assure that a nachos system tick takes a specified amount of time.
//  Wait if nachos ticks run too fast.
//  The amount of time per tick is specified in kernel->millisPerTick.
//  This method only waits if this value is greater than 0.
//  So if you want to deactivate the waiting behaviour, just set the value
//  to 0 for example.
//----------------------------------------------------------------------

void Interrupt::SynchronizeWithRealTime() {
    if (kernel->millisPerTick>0) {
      // calculate the time we should have by now
      long millis = round(kernel->stats->totalTicks * kernel->millisPerTick);
      // control time by waiting (desired time - actual time)
      millisleep(millis - getRealTicks());
    }
}
