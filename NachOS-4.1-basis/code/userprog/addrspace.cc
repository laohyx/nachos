...
AddrSpace::AddrSpace()
{
    pageTable = new TranslationEntry[kernel->NumPhysPages];
    for (int i = 0; i < kernel->NumPhysPages; i++) {
...
// how big is address space?
    ...
    ASSERT(numPages <= kernel->NumPhysPages);	// check we're not trying
